...
def val(val_loader, model):
    ...
    for i, (_, input, target) in pbar:
        ...
        total_images += input.size(0)  # Accumulate number of images processed

    elapsed_time = time.time() - start_time  # Total time for processing
    fps = total_images / elapsed_time  # Calculate FPS

    da_segment_result = (da_acc_seg.avg, da_IoU_seg.avg, da_mIoU_seg.avg)
    ll_segment_result = (ll_acc_seg.avg, ll_IoU_seg.avg, ll_mIoU_seg.avg)

    return da_segment_result, ll_segment_result, fps  # Return FPS along with other metrics
